...
author: "Freja Ipsen, Mathilde Petersen, Sissel Breckling & Josefine Topbjerg"
date: "`r Sys.time()`"
output: 
  html_document:
    toc: TRUE
...
Trying to get text from a single page (e.g. Page 9)
```{r single-page}
got_p9 <- got_text[9]
got_p9
```
...
OK...so we notice that a whole bunch of things show up frequently that we might not be interested in ("a", "the", "and", etc.). Let's remove them. 
...
```{r skip-numbers}
# This code will filter out numbers:
...
# We don't want to include them all in a word cloud. Let's filter to only include the top 100 most frequent?
...
ggplot(data = got_top100, aes(label = word, size = n)) +
  ...
  scale_color_gradientn(colors = c("orange","magenta","blue")) +
  theme_minimal()
```
...
## The most common meaningful words and which emotions we expect will dominate this volume
